...
import custom_comp.BankAccountDateBalanceComparator;
		...
		try (Scanner sc = new Scanner(System.in)) {
			...
			while (!exit) {
				System.out.println("1 . Create A/c 2. Display all accts 3. Check for existence 4. Get A/C Summary\n"
						+ "5 : Funds Transfer 6 : Close A/C 7. Search by Ac Type\n"
						+ " 8. Sort A/cs as per acct no 9. Sort accts as per date n balance 100.Exit");
				try {
					switch (sc.nextInt()) {
					...
					case 6:// Close a/c : public boolean remove(Object o)
						...
						index = accounts.indexOf(new BankAccount(sc.nextInt()));
						if (index == -1)
							throw new BankAccountHandlingException("Invalid Account no !!!!!: A/c Closing failed");
						System.out.println("A/C Closed...." + accounts.remove(index));
					...
					case 7:
						...
						break;
					case 8: // sort a/cs as per acct nos.(asc) : Natural Ordering
						// public static void sort(List<T> list); : Collections class
						Collections.sort(accounts);// Method is incompatible : ArrayList<BankAccount> : List<T>
						break;
					case 9:
						// public static void sort(List<T> list, Comparator<T> c) : Custom Ordering
						// c : instance of the class that imple Comaparator i/f
						Collections.sort(accounts, new BankAccountDateBalanceComparator());
						break;
					case 100:
						exit = true;
						break;
